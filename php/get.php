<?php
    include 'includes/util.inc.php';
    include 'includes/header.php';
    include 'includes/menu.php';

?>

<h1>GET</h1>

<?php
    //la super-globale $_GET est tableau associatif contenant
    // les paramètres fournies dans l'URL

    $country = $_GET['country'];
    $sport = $_GET['sport'];

    echop('Pays demandé : ' . $country);
    echop('Sport demandé : ' . $sport);

    switch (strtolower($country)) { // strtolower function converts a string to lowercase
        case 'italie':
            echo "Siamo molto felici di imparare il PHP";
            include 'italie.php'; // include strtolower($country) . '.php'
            break;
        case 'france':
            echo "Nous sommes très heureux d'apprendre le PHP";
            break;
        case 'roumanie':
            echo "Suntum foarte fericiti sa invatem PHP-ul";
            break;
        case 'espagne':
            echo "Estamos muy felices de apprender el PHP";
            break;
        default:
            echo "Pays inconnu";
            break;
    }

    

?>

<?php include 'includes/footer.php'; ?>